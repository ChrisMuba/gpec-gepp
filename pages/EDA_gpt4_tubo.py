...
df['turnover_date'] = pd.to_datetime(df['Date_de_départ'])
df['Date_de_recrutement'] = pd.to_datetime(df['Date_de_recrutement'])
df['tenure'] = (df['turnover_date'] - df['Date_de_recrutement']).dt.days / 365
df['turnover'] = 1
...
df_turnover_by_dept = df.groupby('Service')['turnover'].sum()
...
df_turnover_by_gender = df.groupby('Genre')['turnover'].sum()
...
df_turnover_by_position = df.groupby('Poste_occupé')['turnover'].sum()
...
df_departure_reasons = df.groupby('Raison_du_départ')['turnover'].sum()
